...
const invalidContextError = 'Invalid context "sepia". Expected one of: light, dark.';
...
describe("context precedence", () => {
  ...
  it("rejects invalid contexts at any provided level", () => {
    expect(() => resolveContext({ resolverContext: "sepia" })).toThrow(invalidContextError);
    expect(() => resolveContext({ paletteContext: "sepia" })).toThrow(invalidContextError);
    expect(() => resolveContext({ systemDefaultContext: "sepia" })).toThrow(invalidContextError);
  });
...
describe("context curve hooks", () => {
  it("returns a frozen hook", () => {
    const hook = createContextCurveHook({ light: 98, dark: 12 });
  ...
  it("preserves an internal frozen copy of curve values", () => {
    const values = { light: 98, dark: 12 } satisfies ContextCurveValues<number>;
