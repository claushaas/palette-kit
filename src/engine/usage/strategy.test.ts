...
const strategyCases = [
  { usage: "fill", strategy: fillUsageStrategy },
  { usage: "visualVocabulary", strategy: visualVocabularyUsageStrategy },
  { usage: "lines", strategy: linesUsageStrategy },
  { usage: "overlays", strategy: overlaysUsageStrategy },
] as const;
...
describe("usage strategies", () => {
  ...
  it("returns structural results without changing intent", () => {
    for (const { usage, strategy } of strategyCases) {
      ...
      expect(result).toEqual({ usage, intent });
