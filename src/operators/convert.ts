...
const normalizePrecision = (value: number) =>
	Math.abs(value) <= CONVERSION_EPSILON ? 0 : value;
...
export function oklabToOklch(
	input: OklabInput,
): ReturnType<typeof normalizeOklch> {
...
export function oklabToLinearRgb(input: OklabInput): LinearRgbColor {
	...
	return {
		alpha,
		b: normalizePrecision(
			-0.0041960863 * l - 0.7034186147 * m + 1.707614701 * s,
		),
		g: normalizePrecision(
			-1.2684380046 * l + 2.6097574011 * m - 0.3413193965 * s,
		),
		r: normalizePrecision(
...
export function linearRgbToOklab(input: LinearRgbInput): OklabColor {
	...
	const l = Math.cbrt(
		0.4122214708 * input.r + 0.5363325363 * input.g + 0.0514459929 * input.b,
	);
	const m = Math.cbrt(
		0.2119034982 * input.r + 0.6806995451 * input.g + 0.1073969566 * input.b,
	);
	const s = Math.cbrt(
		0.0883024619 * input.r + 0.2817188376 * input.g + 0.6299787005 * input.b,
	);

	return {
		a: normalizePrecision(
