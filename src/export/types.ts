...
export function isColorOutput(value: unknown): value is ColorOutput {
	return (
		typeof value === 'string' && (OUTPUTS as readonly string[]).includes(value)
	);
}

export function assertColorOutput(
	value: unknown,
): asserts value is ColorOutput {
	if (!isColorOutput(value)) {
		throw createUnsupportedOutputError(
			String(value),
			formatInvalidOutputError(value),
		);
	}
}
