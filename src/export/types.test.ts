...
const invalidOutputError =
	'Invalid output "css". Expected one of: oklch, oklab, srgb, p3, hex, rgba.';
...
describe('output precedence', () => {
	...
	it('prefers palette output over system default', () => {
		expect(
			resolveOutput({ paletteOutput: 'rgba', systemDefaultOutput: 'oklch' }),
		).toBe('rgba');
	});
	...
	it('rejects invalid outputs at any provided level', () => {
		expect(() => resolveOutput({ resolverOutput: 'css' })).toThrow(
			invalidOutputError,
		);
		expect(() =>
			resolveOutput({ paletteOutput: 'css', resolverOutput: 'hex' }),
		).toThrow(invalidOutputError);
		expect(() =>
			resolveOutput({
				paletteOutput: 'rgba',
				resolverOutput: 'hex',
				systemDefaultOutput: 'css',
			}),
		).toThrow(invalidOutputError);
	});
});
...
describe('output typing', () => {
	it('maps output names to concrete return types', () => {
		...
		expect(rgba).toEqual({ a: 1, b: 255, g: 255, r: 255 } satisfies RgbaColor);
