...
export const createMissingRequiredAxisError = (axis: string, usage: string, message?: string) =>
  new PaletteKitError(
    "MISSING_REQUIRED_AXIS",
    "resolver-input",
...
export const createInvalidRelationTargetError = (relation: string, message?: string) =>
  new PaletteKitError(
    "INVALID_RELATION_TARGET",
    "resolver-input",
    message ?? `Relation "${relation}" target must be a normalized OKLCH color.`,
    { relation },
  );
...
export const createUnsupportedOutputError = (output: string, message?: string) =>
  new PaletteKitError(
    "UNSUPPORTED_OUTPUT",
    "serialization",
