...
const maxStateDelta = (config: ResolverConfig) =>
	Math.max(
		...Object.values(config.stateDeltas as Readonly<Record<State, number>>),
	);
...
describe('resolver preset configs', () => {
	...
	it('rejects invalid levels without fallback', () => {
		for (const config of Object.values(resolverPresetConfigs)) {
			expect(() => config.levelCurves.fill(0 as Level)).toThrow(
				invalidLevelError,
			);
			expect(() => config.levelCurves.lines(0 as Level)).toThrow(
				invalidLevelError,
			);
			expect(() => config.levelCurves.overlays(0 as Level)).toThrow(
				invalidLevelError,
			);
		}
	});
